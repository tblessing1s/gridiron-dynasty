...
- **Phase 2 — Border War (in progress).** Turn the drive into the battle: 3 possessions each, 4 downs, no kicks, play cards, RB/Blocker/Rusher, stats-driven physics, four involvement modes (Play, Watch, Sim — default, since the real-time engine isn't reliable enough to carry a whole battle yet — and Auto-resolve). Two hard-coded rosters, matchup screen, border bar, home-crowd modifier, aftermath with battle XP and fatigue, the raid with franchise-tag protection and the morale penalty (rosters persist for the session). Still to do: stakes scaling, balance pass (red-zone throws score too easily, Blitz is strong).
