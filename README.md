...
**Watch the play.** In SIM, the down's stat math decides everything first, then (if the "WATCH THE PLAY" toggle on the main menu is on, which it is by default) the field plays a few seconds of a scripted replay of that exact result — routes, the throw, the tackle, the sack, the pick return — before the message and detail lines reveal it, so the animation can never disagree with the numbers. Click, tap, or press any key to skip it; a speed picker (1×/1.5×/2×) and an in-battle REPLAY button turn it off or speed it up mid-game. AUTO-RESOLVE and AI-vs-AI battles on the season map never replay.
