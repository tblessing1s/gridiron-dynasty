...
Bugfix found by playtesting the above: an interception's turnover spot was computed from the pre-snap line of scrimmage (`ball_yard`) instead of where the pass actually traveled, so the next possession started as if every pick happened right at the snap — harmless as a text-only result before this session, but now visibly wrong once the replay shows the ball landing `air_yards` downfield first. This bug pre-dated Part D (same `clampi(ball_yard, ...)` pattern already existed); the replay just made it visible for the first time. Fixed in both `football_game._resolve_sim_play` (the user's SIM battle) and `BattleSim._resolve_possession` (auto-resolve and every AI-vs-AI season battle) by spotting the turnover at `ball_yard + air_yards`, matching what `_on_pass_intercepted`'s real-time path already did correctly with the live defender's position. Also tightened the run replay's "hitting the hole" hop so a stuffed or negative run no longer visibly surges past the line of scrimmage before snapping back for the loss (`play_replay._sequence_run`'s `hole_x` now clamps to `minf(los_x + 20.0, final_x)`).
